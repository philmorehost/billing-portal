...
if (!$token) redirect(BASE_URL . '/admin/login.php');

$valid = DB::row("SELECT id FROM admins WHERE password_reset_token=? AND password_reset_expires > NOW()", 's', [$token]);
...
if (is_post() && csrf_verify() && $valid) {
    ...
    elseif ($pw !== $pw2)   { $error = 'Passwords do not match.'; }
        ...
        Auth::resetPassword($token, $pw, 'admins');
...
<title>Reset Password — <?= h($company) ?> Admin</title>
...
.rp-card{background:#fff;border-radius:20px;box-shadow:0 4px 40px rgba(0,0,0,.09);padding:40px;max-width:420px;width:100%;}
.rp-card h2{font-size:20px;font-weight:700;margin-bottom:6px;}
.rp-card p{color:#64748b;font-size:14px;margin-bottom:28px;}
...
.btn-save{background:#0f172a;color:#fff;width:100%;padding:13px;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;}
</style>
</head>
<body>
<div class="rp-card">
  ...
  <?php if ($success): ?><div class="alert-custom alert-success mb-3"><span class="alert-icon">✓</span> <?= h($success) ?> <a href="<?= BASE_URL ?>/admin/login.php" style="color:#0f172a;font-weight:700">Login →</a></div><?php endif ?>
