...
$id=(int)($_SESSION['2fa_pending_admin']??0);
if (!$id) redirect(BASE_URL.'/admin/login.php');
$admin=DB::row("SELECT * FROM admins WHERE id=?",'i',[$id]);
if (!$admin||!$admin['two_factor_enabled']) { unset($_SESSION['2fa_pending_admin']); redirect(BASE_URL.'/admin/login.php'); }
if (is_post()&&csrf_verify()) {
    if (Auth::verify2FA($admin['two_factor_secret'],preg_replace('/\s/','',post('code')))) {
        unset($_SESSION['2fa_pending_admin']); Auth::setAdminSession($admin); redirect(BASE_URL.'/admin/');
...
<div style="margin-top:20px;font-size:13px;color:#64748b"><a href="<?=BASE_URL?>/admin/login.php" style="color:#3b82f6;text-decoration:none">← Back to login</a></div>
</div></body></html>
