...
if(!$id) redirect(BASE_URL.'/reseller/login.php');
$client=DB::row("SELECT * FROM clients WHERE id=?",'i',[$id]);
if(!$client||!$client['two_factor_enabled']){unset($_SESSION['2fa_pending_client']);redirect(BASE_URL.'/reseller/login.php');}
if(is_post()&&csrf_verify()){
    if(Auth::verify2FA($client['two_factor_secret'],preg_replace('/\s/','',post('code')))){
        unset($_SESSION['2fa_pending_client']);
        Auth::setClientSession($client);
        $r=DB::row("SELECT id FROM resellers WHERE client_id=? AND status='active'",'i',[$id]);
...
?><!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>2FA — Reseller</title>
...
<input type="text" name="code" class="ci" placeholder="000000" maxlength="6" pattern="[0-9]{6}" inputmode="numeric" autofocus required>
<button type="submit" class="bv">Verify →</button></form>
<div style="margin-top:20px;font-size:13px"><a href="<?=BASE_URL?>/reseller/login.php" style="color:#3b82f6;text-decoration:none">← Back to login</a></div>
</div></body></html>
